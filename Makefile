BUILD = build
MAKEFILE = Makefile
OUTPUT_FILENAME = Die_grosse_Reiterschlacht_bei_BrandyStation
TITLE_NAME = "Die grosse Reiterschlacht bei BrandyStation"
...
$(BUILD)/pdf/$(OUTPUT_FILENAME).pdf: $(MAKEFILE) $(METADATA_PDF) $(CHAPTERS_HTML_PDF) $(CSS_FILE) $(IMAGES) $(COVER_IMAGE) $(METADATA_PDF) $(PREFACE_EPUB)
	mkdir -p $(BUILD)/pdf
	cp  *.css  $(IMAGES_FOLDER)
	cp  $(IMAGES_FOLDER)/Die_grosse_Reiterschlacht_bei_BrandyStation_*.jpg .
	...
	rm  $(IMAGES_FOLDER)/*.css
	rm Die_grosse_Reiterschlacht_bei_BrandyStation_*.jpg 
	rm logo*.jpg 
	rm cover*.jpg 
